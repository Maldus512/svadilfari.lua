...
local new = function(args)
    ...
    ---@alias BuildArgs {inputs: (string | string[])?, target: (string | fun(string): string), implicit: (string | string[])?, variables: {[string]: string}?} | string
    ...
    local build = function(ruleName, args)
        assert(args.inputs == nil or type(args.inputs) == "string" or type(args.inputs) == "table",
            "inputs parameter must be an optional string or list, not " .. type(args.inputs))
        args.inputs = type(args.inputs) == "table" and args.inputs or { args.inputs }
    ...
    local linkElf = function(args)
        ...
        assert(args.libs == nil, type(args.libs) == "string" or type(args.libs) == "table",
            "The libs must be an optional string or a list, not " .. type(args.libs))
